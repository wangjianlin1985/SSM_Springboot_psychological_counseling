...
public class TeacherController extends BaseController {
	...
	public void list(String teacherNo,String teacherName,String comeDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (teacherNo == null) teacherNo = "";
		if (teacherName == null) teacherName = "";
		if (comeDate == null) comeDate = "";
		if(rows != 0)teacherService.setRows(rows);
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, teacherName, comeDate, page);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, teacherName, comeDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String teacherNo,String teacherName,String comeDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (teacherNo == null) teacherNo = "";
		if (teacherName == null) teacherName = "";
		if (comeDate == null) comeDate = "";
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, teacherName, comeDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, teacherName, comeDate);
	    /*获取到总的页码数目*/
	    int totalPage = teacherService.getTotalPage();
	    ...
	    request.setAttribute("comeDate", comeDate);
	...
	public void OutToExcel(String teacherNo,String teacherName,String comeDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(teacherNo == null) teacherNo = "";
        if(teacherName == null) teacherName = "";
        if(comeDate == null) comeDate = "";
        List<Teacher> teacherList = teacherService.queryTeacher(teacherNo,teacherName,comeDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Teacher信息记录"; 
        String[] headers = { "教师工号","教师姓名","教师性别","教师照片","入职日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<teacherList.size();i++) {
        	Teacher teacher = teacherList.get(i); 
        	dataset.add(new String[]{teacher.getTeacherNo(),teacher.getTeacherName(),teacher.getTeacherSex(),teacher.getTeacherPhoto(),teacher.getComeDate()});
        }
        /*
